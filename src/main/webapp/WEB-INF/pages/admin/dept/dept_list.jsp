...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 管理员管理<span class="c-gray en">&gt;</span> 部门管理<span class="c-gray en">&gt;</span>部门列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="Hui-article">
		<article class="cl pd-20">
		<form action="${pageContext.request.contextPath}/admin/admin-dept.do" method="post" id="checkForm">
			<div class="text-c"> 创建日期：
				...
				<input type="text" class="input-text" style="width:250px" placeholder="输入部门名称" id="dName" name="dName" value="${dName}">
				<span class="select-box" style="width: 120px;">
				状态：
	  				<select name="dStatus" class="select"  size="1" name="demo1" style="width: 55px;">
				...
				<button type="button" class="btn btn-success" id="checkAffix" name="checkAffix" onclick="shouAffix();"><i class="Hui-iconfont">&#xe665;</i> 搜部门</button>
			</div>
		</form>
			<div class="cl pd-5 bg-1 bk-gray mt-20" style="line-height: 30px;">
				<span class="l"> 
<!-- 				<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>  -->
				<a href="javascript:;" onclick="my_dept_add('添加部门','${pageContext.request.contextPath}/admin/dept/add-dept.do','900','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加部门</a> </span>
			...
			<div class="mt-20">
				...
				<c:forEach var="item" items="${pageView.listData }" varStatus="vs">
					<tr class="text-c">
						...
						<td>${item.dDesc }</td>
						<td>
							<fmt:formatDate value="${item.dCreateTime  }" pattern="yyyy-MM-dd HH:mm"/>
						</td>
						<%-- <td>
						...
						<td>
							<c:if test="${item.dStatus == 1 }"><i class="Hui-iconfont">&#xe615;</i>启用</c:if>
							<c:if test="${item.dStatus == 0 }"><i class="Hui-iconfont">&#xe601;</i>禁用</c:if>
						</td>
						<td class="td-manage">
							<a title="查看" style="text-decoration:none" onClick="show_item('查看','${pageContext.request.contextPath}/admin/dept/add-dept.do?dId=${item.dId }')" href="javascript:;" ><i class="Hui-iconfont">&#xe6df;</i></a>
							<a title="删除" href="javascript:;" onclick="items_del(this,'${item.dId }')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
						 </td>
					</tr>
				</c:forEach>
				</tbody>
...
function items_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		...
		$.ajax({
			url:"${pageContext.request.contextPath}/admin/dept/dept_del.do",
            type:"post",
            data:{dId:id},
